...
def euler_sampler_multistep(
    ...
    step_condition=None,
    t_steps=None,
    num_future_steps=4,  # New parameter for multi-step prediction
    **kwargs
):  
    """
    Modified Euler sampler for models that predict multiple future steps at once.
    
    Args:
        num_future_steps: Number of future steps the model predicts simultaneously
    """
    ...
    # Main sampling loop - now we can skip steps when using multi-step predictions
    ...
    while i < len(t_steps) - 1:
        ...
        # Determine how many steps ahead we can predict
        remaining_steps = len(t_steps) - 1 - i
        steps_to_predict = min(num_future_steps, remaining_steps)
        
        # Get the target times for multi-step prediction
        target_times = t_steps[i+1:i+1+steps_to_predict]
        
        # Euler step with multi-step prediction
        # use_afs = afs and (((not train) and i == 0) or (train and step_idx == 0))
        use_afs = False
        if use_afs:
            # For AFS, we can only do single step
            ...
            # print("""for now denoised is of shape [B, C, H, W] we will copy it to [B, num_future_steps, C, H, W]""")
            # denoised_multistep = denoised_multistep.unsqueeze(1).repeat(1, num_future_steps, 1, 1, 1)
            # print(denoised_multistep.shape, "  shape for denoised_multistep after repeat")
            #reshape denoised_multistep to [B, num_future_steps, C, H, W]

            CHANNEL_AXIS = 1
            # denoised_multistep =denoised_multistep.view(-1,4,net.model.img_channels,net.model.img_resolution,net.model.img_resolution)
            denoised_multistep =denoised_multistep.view(-1,4,3,32,32)
            # x1 = denoised_multistep[:,0]
            # x2 = denoised_multistep[:,1]
            # x3 = denoised_multistep[:,2]
            # x4 = denoised_multistep[:,3]
            # print("diff")
            # print((x1-x2).abs().mean())
            # print((x2-x3).abs().mean())
            # print((x3-x4).abs().mean())
            # print(x1.std())
            # print(x2.std())
            # print(x3.std())
            # print(x4.std())
            ...
            # denoised_multistep = torch.chunk(denoised_multistep, denoised_multistep.shape[CHANNEL_AXIS], CHANNEL_AXIS)
            # print(denoised_multistep.shape, "  shape for denoised_multistep")
            # Apply Euler steps for each predicted step
            x_current = x_cur
            for step_offset in range(steps_to_predict):
                # print(step_offset)
                t_current = t_steps[i + step_offset]
                t_next = target_times[step_offset]
            ...
            # Skip ahead by the number of steps we predicted
            i += steps_to_predict

    if denoise_to_zero and i < len(t_steps):
        t_final = t_steps[-1]
        denoised_final = get_denoised(net, x_next, t_final, 
                                               class_labels=class_labels, 
                                               condition=condition, 
                                               unconditional_condition=unconditional_condition,
                                               num_future_steps=1)
        x_next = denoised_final[:, 0]  # Take the first (and only) prediction
        if return_inters:
            inters.append(x_next.unsqueeze(0))
